...
public final class MainActivity extends AppCompatActivity
  ...
  public boolean onQueryTextChange(final String query) {
    List<Summary> one = Summary.filter(courses, query);
    listAdapter.edit().replaceAll(one).commit();
    Log.v("t", "The method is called");
    return true;
  }
  ...
  public void onCourseClicked(final Summary course) {
    ...
    String courseAsString = new String();
    try {
      courseAsString = objectMapper.writeValueAsString(course);
    } catch (JsonProcessingException e) {
      Log.d(TAG, "exception");
    }
    Intent startCourseActivity = new Intent(this, CourseActivity.class);
    startCourseActivity.putExtra("COURSE", courseAsString);
